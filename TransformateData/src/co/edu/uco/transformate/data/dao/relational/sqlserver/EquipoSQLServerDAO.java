...
public class EquipoSQLServerDAO extends SQLDAO<EquipoEntity> implements EquipoDAO{
	...
	public void create(EquipoEntity entity) {
		final var sqlStatement = "INSERT INTO Equipo (identificador, nombre,descripcion,zona) ";
		try (var preparedStatement = getConnection().prepareStatement(sqlStatement)) {
			preparedStatement.setObject(1, entity.getIdentificador());
			preparedStatement.setObject(2, entity.getNombre());
			preparedStatement.setObject(3, entity.getDescripcion());
			preparedStatement.setObject(4, entity.getZonaDTO());
	...
	protected String prepareSelec() {
		return "SELECT identificador, nombre,descripcion,zona ";
	}
	...
	protected String prepareWhere(EquipoEntity entity, List<Object> parameters) {
		...
		if (!UtilObject.isNull(entity)) {
			...
			if (UtilObject.isEmpty(entity.getZonaDTO())) {
				parameters.add(entity.getZonaDTO());
				where.append(setWhere ? where() : and()).append(" zona=? ");
	...
	protected List<EquipoEntity> executeQuery(PreparedStatement preparedStatement) {
		final List<EquipoEntity> resultEntities = new ArrayList<>();
		try (var resultSet = preparedStatement.executeQuery()) {
			while (resultSet.next()) {
				final var entityTmp = new EquipoEntity(resultSet.getObject("identificador", UUID.class),
				
						resultSet.getString("nombre"), resultSet.getString("descripcion"),
						
						resultSet.getObject("zona", ZonaEntity.class));
