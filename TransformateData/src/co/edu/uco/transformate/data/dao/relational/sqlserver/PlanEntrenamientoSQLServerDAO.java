...
public class PlanEntrenamientoSQLServerDAO extends SQLDAO<PlanEntrenamientoEntity> implements PlanEntrenamientoDAO{

	@Override
	public void create(PlanEntrenamientoEntity entity) {
		final var sqlStatement = "INSERT INTO PlanEntrenamiento (identificador, nombre, descripcion, nivelDificultad) ";
		try (var preparedStatement = getConnection().prepareStatement(sqlStatement)) {
			preparedStatement.setObject(1, entity.getIdentificador());
			preparedStatement.setObject(2, entity.getNombre());
			preparedStatement.setObject(3, entity.getDescripcion());
			preparedStatement.setObject(4, entity.getNivel());
	...
	@Override
	public List<PlanEntrenamientoEntity> read(PlanEntrenamientoEntity entity) {
	...
	@Override
	public void update(PlanEntrenamientoEntity entity) {
		final var sqlStatement = "UPDATE PlanEntrenamiento SET nombre=?,descripcion=?,nivelDificultad=? WHERE Identificador = ?";
		try (var preparedStatement = getConnection().prepareStatement(sqlStatement)) {
	...
	protected String prepareSelec() {
		return "identificador, nombre, descripcion, nivelDificultad ";
	}

	@Override
	protected String prepareFrom() {
		return "FROM PlanEntrenamiento ";
	}


	@Override
	protected String prepareWhere(PlanEntrenamientoEntity entity, List<Object> parameters) {
		...
		if (!UtilObject.isNull(entity)) {
			...
			if (UtilObject.isEmpty(entity.getNivel())) {
				parameters.add(entity.getNivel());
				where.append(setWhere ? where() : and()).append(" tipoDocumneto=? ");
	...
	@Override
	protected List<PlanEntrenamientoEntity> executeQuery(PreparedStatement preparedStatement) {
		final List<PlanEntrenamientoEntity> resultEntities = new ArrayList<>();
		try (var resultSet = preparedStatement.executeQuery()) {
			while (resultSet.next()) {
				final var entityTmp = new PlanEntrenamientoEntity(resultSet.getObject("identificador", UUID.class),
						 resultSet.getString("nombre"),resultSet.getString("descripcion"),
						resultSet.getObject("nivelDificultad",NivelDificultadEntity.class));
