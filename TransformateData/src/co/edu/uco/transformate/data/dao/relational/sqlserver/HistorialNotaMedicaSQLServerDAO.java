...
public class HistorialNotaMedicaSQLServerDAO extends SQLDAO<HistorialNotaMedicaEntity> implements HistorialNotaMedicaDAO{

	@Override
	public void create(HistorialNotaMedicaEntity entity) {
		final var sqlStatement = "INSERT INTO HistorialNotaMedica(identificador, fecha,descripcion, nombreMiembro) ";
		try (var preparedStatement = getConnection().prepareStatement(sqlStatement)) {
			...
			preparedStatement.setObject(4, entity.getNombreMiembroString());
	...
	@Override
	public List<HistorialNotaMedicaEntity> read(HistorialNotaMedicaEntity entity) {
	...
	protected String prepareFrom() {
		return "FROM HistorialNotaMedica ";
	}

	@Override
	protected String prepareWhere(HistorialNotaMedicaEntity entity, List<Object> parameters) {
		...
		if (!UtilObject.isNull(entity)) {
			...
			if (UtilObject.isEmpty(entity.getFechaDate())) {
			...
			if (UtilObject.isEmpty(entity.getNombreMiembroString())) {
				parameters.add(entity.getNombreMiembroString());
	...
	@Override
	protected List<HistorialNotaMedicaEntity> executeQuery(PreparedStatement preparedStatement) {
		final List<HistorialNotaMedicaEntity> resultEntities = new ArrayList<>();
		try (var resultSet = preparedStatement.executeQuery()) {
			while (resultSet.next()) {
				final var entityTmp = new HistorialNotaMedicaEntity(resultSet.getObject("identificador", UUID.class),
