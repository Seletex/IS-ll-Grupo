...
public class MembresiaAssembler implements Assembler<MembresiaDomain, MembresiaDTO, MembresiaEntity> {
	...
	public MembresiaEntity toEntityFromDomain(MembresiaDomain domain) {
		return new MembresiaEntity(domain.getIdentificador(),TipoMembresiaEntity.create(UtilUUID.DEFAULT_UUID, UtilText.EMPTY),domain.getFechaInicio(),domain.getFechaFin()
				);
	}

	@Override
	public MembresiaDomain toDomainFromEntity(MembresiaEntity entity) {
		return new MembresiaDomain(entity.getIdentificador(),TipoMembresiaAssembler.getInstance().toDomainFromEntity(entity.getTipoMembresiaDTO()),entity.getFechaInicio(),entity.getFechaFin()
				);
	}
